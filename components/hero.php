<?php global $lang; ?>
    <section id="hero" class="hero">
        <div class="container hero-container">
            <div class="hero-text">
                <p class="greeting type-effect"><?php echo $lang['hero_greeting']; ?></p>
                <h1 class="glitch type-effect"><?php echo $lang['hero_name']; ?></h1>
                <h2 class="subtitle type-effect"><?php echo $lang['hero_subtitle']; ?></h2>
                <p class="hero-desc">
                    <?php echo $lang['hero_desc']; ?>
                </p>
                <div class="hero-cta">
                    <a href="#projects" class="btn btn-primary"><?php echo $lang['hero_cta']; ?></a>
                    <a href="assets/LyHongPhat_SoftwareEngineer_Resume.pdf" class="btn btn-outline" target="_blank" download="LyHongPhat_CV.pdf"><?php echo $lang['hero_download_cv']; ?></a>
                </div>
            </div>
        </div>
    </section>
